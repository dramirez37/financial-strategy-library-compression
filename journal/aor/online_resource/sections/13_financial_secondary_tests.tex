...
The empirical layer audits two declared tuple-grammar libraries ex post. It
does not observe institutional source-code, validation, or knowledge
inventories and does not establish that deleting an active row destroys a
separately archived artifact. The primary comparison is innovation-safe versus
frontier-only pruning; coverage ranking is a secondary stress test. The
information-set boundary is summarized in
the repository's locked experiment protocols and applies to every definition
and result below.
...
For net validation returns \(r_t\), the implemented profile coordinate is
\[
 \widehat j=\AORFinancialAnnualization\,\overline r
 -\frac12(\AORFinancialRiskAversion)(\AORFinancialAnnualization)
  \widehat{\operatorname{Var}}(r).
\]
The variance uses the sample denominator when at least two observations are
available, and net returns deduct a one-way turnover charge of
\AORFinancialPrimaryCostBps\ basis points, with the locked
\AORFinancialSensitivityBps\ basis-point sensitivity grid. The score is an
annualized estimated mean--variance audit quantity, not a true opportunity or
a measured institutional cost. The original terminal and annual equality gates
use tolerances \(\AORTerminalFrontierTolerance\) and
\(\AORAnnualFrontierTolerance\), respectively. Journal algorithm instances
freeze those profile tables and recheck the frozen coordinates exactly.
...
equality. The quantity \(Q_a(L')\) is the \emph{ex post catalog-maximum
witness}. It records the maximum realized score among descendants enabled by
the declared tuple grammar. It is evaluated only after pruning decisions are
fixed and is neither a forecast, a policy score, an independent validation
test, nor a deployable selection criterion. Equality of \(Q_a\) for any two
exactly safe libraries follows mechanically because closure equality makes
their enabled catalogs identical.
...
computed from held-out \(Q_a\), and the ``supports the catalog maximum'' flag also
...
\(113=113\). Thus the two audits give an ex post witness that frontier-only
preservation changed the enabled catalog and its realized maximum under these
audit-specific scores. This is not independent evidence for safe compression:
safe-library equality is already implied by closure equality. Nor is the
catalog maximum corrected for testing or selecting across many descendants
\citep{SullivanTimmermannWhite1999,White2000,HarveyLiuZhu2016}. The diagnostic
neither changes either pruning rule nor turns realized quality into information
known at deletion time.

Each safe library has exactly one positive ex post catalog carrier: GLD has
\((O,G)=(0,0.0016)\) and UNG has \((O,G)=(0,0.0534)\).  Both uniquely support
the audit's catalog-maximizing descendant. With only two carriers, the structural
...
\begin{table}[htbp]
  ...
  \input{tables/financial_design_summary}
...
\subsection{Journal algorithm comparison and locked retrospective weight replay}
\label{sec:or-financial-algorithm-robustness}

The journal extension evaluates the common algorithm suite against the same
two source instances without changing their information boundary. No held-out
quantity enters a retention weight, preprocessing rule, algorithm choice,
warm start, pruning order, or objective. Held-out opportunity diagnostics are
computed only after selection and are reported in the terminal and annual
units separately.

\input{tables/financial_instance_summary}

The nonuniform burden comparison below is generated from the committed public
aggregate result rows. Exact reconstructed burden, not a solver's displayed
floating-point objective, is the reported quantity.

\input{tables/financial_algorithm_burdens}

The three replayed schedules are equal active-strategy burden,
validation-computation burden, and governance complexity constructed only from
fields available at compression time. The resource protocol records formulae,
units, permitted inputs, prohibited held-out inputs, tie handling, range, and
missing-data rules for a reproducible post-outcome lock. The schedules and some
earlier outcomes predate that lock, so this is not first-look prospective
evidence. Schedule agreement and the
global-versus-stepwise burden difference are shown without pooling terminal and
annual opportunity units.

\input{tables/financial_robustness_summary}

For each audit--schedule pair, exact preprocessing reaches an empty residual
cover and the lifted selection passes exact mandatory, tagged-coverage,
frontier, closure, and burden checks. Section~S14 states the evidence boundary:
this is a global optimum certificate from human preprocessing propositions plus
exact finite computation, not Lean verification and not a solver-status claim.
The public aggregate selections, identity-overlap comparisons, rank-stability
rows, and unique-carrier records are under
\path{experiments/results/summaries/financial_resource_optimization_weight_robustness_*.csv}.

Raw CRSP/WRDS observations and row-level licensed transformations are excluded
from this resource. Public tables contain only redistributable aggregates and
library identifiers allowed by the repository data-rights policy. These
retrospective audits illustrate burden allocation within the declared grammar,
not population resource savings or causal, forecasting, alpha, or deployable-
performance claims. Both source instances preprocess to empty residual models,
and the weights are additive engineering indices rather than measured dollars,
hours, or compute consumption. Their current snapshot and endpoint-survivor
construction create population-level survivorship and revision limits despite
the within-origin outcome firewall. A stronger licensed-data study therefore
requires a new prospective lock with multiple point-in-time decision origins,
origin-eligible and discontinued securities where available, documented module
artifacts, independently calibrated maintenance costs, and nonempty residual
cases retained rather than selected after preprocessing.
...
\begin{figure}[htbp]
  \centering
  \input{figures/financial_coverage_comparison}
