module AuditAlgorithmicCompressionFinalV1

using SHA: sha256
using StrategyInnovation
using TOML

include(joinpath(@__DIR__, "run_algorithmic_compression_final_v1.jl"))
...
const AUDIT_SCHEMA_VERSION = "algorithmic-compression-final-audit-v1"
...
function _registered_expected(readiness)
    ...
    for spec in readiness.registries.instances
        ...
        for unit in AlgorithmicCompressionFinalV1._registered_run_units(spec)
            push!(rows, (; spec, unit))
        end
    end
    sort!(rows; by = row -> (
...
function _execution_record(environment, execution_lock, total, accepted, unsuccessful, status_counts)
    ...
    return """# Registered Algorithmic Compression Benchmark v1 — execution record
...
function _result_audit_report(summary, status_counts, manifest_hash)
    ...
    return """# Registered Algorithmic Compression Benchmark v1 — result audit
...
function _failure_report(failures, errors)
    io = IOBuffer()
    println(io, "# Registered Algorithmic Compression Benchmark v1 — failure log")
    println(io)
...
function audit_final_benchmark()
    ...
    errors = String[]
    for path in (
        AlgorithmicCompressionFinalV1.EXECUTION_LOCK_PATH,
        AlgorithmicCompressionFinalV1.ENVIRONMENT_PATH,
        AlgorithmicCompressionFinalV1.RUN_STATUS_PATH,
        joinpath(RESULTS_ROOT, "execution_state.toml"),
    )
    ...
    environment = TOML.parsefile(AlgorithmicCompressionFinalV1.ENVIRONMENT_PATH)
    ...
    for row in expected
        spec = row.spec
        unit = row.unit
        path = AlgorithmicCompressionFinalV1._record_path(
            spec.instance_id,
            unit.algorithm,
            unit.variant,
        )
    ...
    for spec in readiness.registries.instances
        ...
        if spec.exact_enumeration_required && spec.exact_dp_required
            ...
            for variant in (:full_fixed_point, :mandatory_only)
                record_path = AlgorithmicCompressionFinalV1._record_path(
                    spec.instance_id,
    ...
    for (instance_id, rows) in certificates
        ...
        payload = Dict{String,Any}(
            "schema_version" => "algorithmic-compression-final-instance-audit-v1",
...
function main(args = ARGS)
    isempty(args) || error("usage: audit_algorithmic_compression_final_v1.jl")
...
if abspath(PROGRAM_FILE) == @__FILE__
    AuditAlgorithmicCompressionFinalV1.main()
end
