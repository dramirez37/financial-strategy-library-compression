...
search over feasible sublibraries can matter.  More sharply, a parameterized
identity-closure family gives the specific heaviest-safe-first rule an
unbounded output-to-optimum burden ratio.  The same family also shows that
one-deletion irreducibility alone has no constant burden guarantee, but it does
not condemn every deletion order: deleting a singleton first reaches the
bundle-only optimum.

The complexity theorem makes that distinction algorithmic. Under identity
closure, exact safe compression is weighted hitting set on frontier-attainer
and module-carrier obligations; the decision problem is NP-complete and the
minimum-weight problem is NP-hard. Exact verification of a returned library is
therefore much easier than establishing global optimality in general. The
bounded greedy and Pareto-beam comparisons illustrate possible search tools,
but their benchmark outcomes provide neither a positive approximation
guarantee nor a large-scale runtime claim.  The worst-case theorem is a
negative guarantee for one named deletion rule, not a performance claim for
all certified heuristics or forward cover constructions.
...
extensions are a separately registered large-library branch-and-cut study with
complete generative separation, structural conditions that justify
approximation guarantees, endogenous information processing, continuous
belief spaces, and endogenous verification of newly generated strategies.
Each would enlarge the choice set while preserving the paper's organizing
question: how much resource burden can be removed without sacrificing the
operational and inventive capacity of the library?
\clearpage
