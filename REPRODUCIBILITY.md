...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 004 command topology on eight synthetic lanes with bounded heavy-stage concurrency, flushed stage progress, and independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
...
002 remain immutable. Execution Amendment 004 binds packed exact preprocessing,
...
`EXECUTION_LOCK_004.json` is the active successor lock.
