\section{Frozen Registered Randomized Library Study (N=1024)}
\label{sec:supp-randomized-study}

\begin{center}
\fbox{\parbox{0.93\linewidth}{\textbf{Historical evidence boundary.}
This is the original frozen \(N=1024\) randomized-library study inherited
unchanged from the preprint program. Its registry, seeds, design locks,
amendments, and committed results predate and are distinct from the Registered
Algorithmic Compression Benchmark v2 in Section~S12.}}
\end{center}
...
comparison behind the historical randomized results reported in the preprint
program. The
...
\begin{table}[htbp]
  \centering
  \small
  \input{tables/numerical_mechanism_summary}
...
aggregated into interaction-sign frequencies; Section~S8 reports the
...
in the registered artifacts indexed in Section~S16.
...
and channel arcs are indexed in Section~S16.

\begin{figure}[htbp]
  \centering
  \input{figures/dynamic_research_policy_regions}
  ...
  points selecting research in a converged raw-derived Float64 solve; light
  unhatched spans denote Continue and diagonal hatching denotes research.
  Filled spans summarize the
