...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 023 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, phase-specific unused-field, terminal-delisting-pending, count-only postdecision-profile-adequacy, Julia 1.12 key-shape, exact MIP warm-start substitution, mixed Lock-005/Lock-020/Lock-022 checkpoint recovery, precomputed serialized-file hash reuse, exact environment-successor recovery, exact projection-provenance reconstruction, and analysis-partition rematerialization fixtures, with independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
| `make aor-financial-panel-v1-run` | Run or resume the locked point-in-time CRSP strategy-library panel using Julia 1.12.6, three concurrent source-file scans, eight stable job/analysis lanes, at most two simultaneous memory-heavy stages, single-threaded BLAS and HiGHS models, and atomic checkpoints. | Long; empirical construction and 1,260 registered algorithm rows are host dependent. | Local licensed CRSP/WRDS delivery at `ALGOLIB_CRSP_ROOT` or the registered default. | Ignored Snappy-Parquet source partitions, origin-scoped source instances or terminal construction-failure slots, resumable algorithm checkpoints, 1,260 terminal algorithm rows, solver logs, structural and result audits, postdecision diagnostics, resumable Parquet analysis, and an analysis audit. |
| `make aor-financial-panel-v1-continue` | Starting from the passing structural audit, run or resume postdecision evaluation under Lock 023, full result audit, eight-thread Parquet analysis, and the independent analysis audit. Amendments 019--023 replace only the 94 exact matching historical MIP projection failures, prevalidate mixed checkpoint provenance, reuse all 108 terminal solver candidates, exactly reconstruct their omitted warm-start projection provenance without solver reruns, require exact MIP--DP burden agreement, refresh all postdecision records, and rematerialize all analysis partitions. Terminal `DP` and insufficient-profile cases remain explicit origin-wide unavailable secondary diagnostics rather than being imputed or deleted. | Medium to long on the first corrected pass; restart cost is bounded by terminal checkpoints, per-instance records, and Parquet partitions. | Local licensed CRSP/WRDS delivery plus completed ignored structural artifacts. | Three local-only source partitions, 180 postdecision records including explicit unavailable records, result audit, 180 rematerialized analysis partitions, compacted analysis tables, and analysis audit. |
...
# Continue the existing completed structural run through audited analysis.
ALGOLIB_CRSP_ROOT=/absolute/path/to/licensed/source make aor-financial-panel-v1-continue
```

Progress bars report terminal counts plus flushed per-lane stages and a
30-second heartbeat; they do not display financial outcomes. Structural return maps are scoped to
...
retained as the immutable record of the initial ingestion failure. Execution
...
002 remain immutable. Execution Amendments 004--010 bind packed exact preprocessing,
...
`EXECUTION_LOCK_010.json` is the active successor lock. Lock 005 adds validated reuse
...
dispatch to the unchanged independent audit after all structural results are
...
the registered job set, audit predicates, or scientific artifacts. Lock 009
uses a deterministic eight-lane audit, canonical serialized-file hashes, and
durable progress for both audit stages; financial analysis remains a separate
registered execution phase. Lock 010 makes concurrent progress output strictly
monotone without changing the parallel computations or audit predicates.
