module LockFinancialStrategyLibraryPanelV1Execution007
...
const Lock006 = LockFinancialStrategyLibraryPanelV1Execution006

export create_execution_lock_007,
       dry_run,
       main,
       validate_execution_design_007,
       verify_execution_lock_007,
       verify_lock_text
...
const PREDECESSOR_LOCK_PATH = joinpath(EXPERIMENT_ROOT, "EXECUTION_LOCK_006.json")
...
const PREDECESSOR_AGGREGATE =
    "a6cb890bc81d5a16e0bd7be68dbe6d83948d421962e83b1c24152806086463bf"
const PREDECESSOR_FILE_SHA256 =
    "87043a287d54bc4bccaba09fd7cb9410529a086f8098ddf85081612a6ee040ec"

const REQUIRED_FILES = (
    Lock006.REQUIRED_FILES...,
    "experiments/financial_strategy_library_panel_v1/EXECUTION_LOCK_006.json",
    "experiments/financial_strategy_library_panel_v1/amendments/AMENDMENT_007.md",
    "experiments/financial_strategy_library_panel_v1/amendments/EXECUTION_AMENDMENT_007.toml",
    "julia/scripts/lock_financial_strategy_library_panel_v1_execution_007.jl",
)
...
function _hashes()
    length(REQUIRED_FILES) == length(unique(REQUIRED_FILES)) ||
        error("Lock 007 input list contains duplicates")
    for relative in REQUIRED_FILES
        isfile(joinpath(REPOSITORY_ROOT, relative)) || error(
            "missing Lock 007 input: $relative",
        )
    end
    return Dict(
...
function _verify_predecessor()
    isfile(PREDECESSOR_LOCK_PATH) || error("Execution Lock 006 is absent")
    predecessor_text = read(PREDECESSOR_LOCK_PATH, String)
    occursin("\"aggregate_sha256\": \"$PREDECESSOR_AGGREGATE\"", predecessor_text) ||
        error("historical Lock 006 aggregate differs")
    _sha256_file(PREDECESSOR_LOCK_PATH) == PREDECESSOR_FILE_SHA256 ||
        error("historical Lock 006 file hash differs")
    return nothing
end

function validate_execution_design_007()
    _verify_predecessor()
    amendment = TOML.parsefile(AMENDMENT_PATH)
    amendment["schema_version"] ==
    "financial-strategy-library-panel-execution-amendment-v7" ||
        error("unexpected Amendment 007 schema")
    amendment["amendment_id"] == "AMENDMENT_007" ||
        error("unexpected Amendment 007 identifier")
    amendment["predecessor_amendment_id"] == "AMENDMENT_006" ||
        error("unexpected Amendment 007 predecessor")
    for (key, expected) in (
        ...
        amendment[key] == expected || error("Amendment 007 disclosure changed: $key")
    end
    for key in (
        ...
        amendment[key] === false || error("Amendment 007 declaration changed: $key")
    end
    amendment["audit_key_shape"]["expected_key_count"] == 180 ||
        error("expected audit key count changed")
    amendment["audit_key_shape"]["representation"] == "sorted Vector{String}" ||
        error("audit key representation changed")
    amendment["dispatch"]["function_binding_lookup"] ==
    "Base.invokelatest(getfield, audit_module, function_name)" ||
        error("latest-world function lookup changed")
...
function _assert_prelock_state(config)
    ...
    count(path -> startswith(path, "origin_builds/"), data_files) == 12 ||
        error("Lock 007 requires 12 origin metadata records")
    count(path -> startswith(path, "origin_failures/"), data_files) == 8 ||
        error("Lock 007 requires 8 origin failure records")
    length(data_files) == 20 || error("unexpected local-data artifacts predate Lock 007")
    count(path -> startswith(path, "instances/"), result_files) == 108 ||
        error("Lock 007 requires 108 serialized instances")
    count(path -> startswith(path, "preparation_failures/"), result_files) == 72 ||
        error("Lock 007 requires 72 preparation-failure slots")
    structural = filter(path -> startswith(path, "structural/"), result_files)
    length(structural) == 180 || error("Lock 007 requires 180 structural records")
    valid_count = 0
    ...
    for relative in structural
        ...
        get(payload, "terminal", false) === true ||
            error("a nonterminal structural record predates Lock 007: $relative")
    ...
    valid_count == 108 || error("Lock 007 valid structural count differs")
    preparation_failure_count == 72 || error("Lock 007 preparation-failure count differs")
    count(path -> startswith(path, "checkpoints/") && endswith(path, ".toml"), result_files) == 756 ||
        error("Lock 007 requires 756 algorithm checkpoints")
    count(path -> startswith(path, "solver_logs/"), result_files) == 14 ||
        error("Lock 007 requires the disclosed 14 solver logs")
    any(path -> startswith(path, "postdecision/"), result_files) &&
        error("postdecision results predate Lock 007")
    "STRUCTURAL_AUDIT.toml" in result_files && error("structural audit predates Lock 007")
    "RESULT_AUDIT.toml" in result_files && error("final audit predates Lock 007")
    ...
    get(environment, "execution_lock_aggregate_sha256", "") == PREDECESSOR_AGGREGATE ||
        error("environment does not belong to Lock 006")
    isempty(_relative_files(public_results)) || error("public results predate Lock 007")
    return nothing
end
...
function _render_lock(hashes)
    ...
    return """{
  "schema_version": "financial-strategy-library-panel-execution-lock-v7",
  "experiment_id": "financial-strategy-library-panel-v1",
  "amendment_id": "AMENDMENT_007",
  ...
  "audit_key_count": 180,
  "audit_key_representation": "sorted Vector{String}",
  "latest_world_function_binding_lookup": true,
...
function verify_lock_text(text::AbstractString, hashes = _hashes())
    validate_execution_design_007()
    occursin("\"schema_version\": \"financial-strategy-library-panel-execution-lock-v7\"", text) ||
        error("unexpected or missing Lock 007 schema")
    occursin("\"predecessor_execution_lock_aggregate_sha256\": \"$PREDECESSOR_AGGREGATE\"", text) ||
        error("Lock 007 predecessor aggregate differs")
    for declaration in (
        ...
        occursin("\"$declaration\": false", text) ||
            error("invalid Lock 007 declaration: $declaration")
    end
    for path in keys(hashes)
        occursin("\"$path\": \"$(hashes[path])\"", text) ||
            error("Lock 007 mismatch: $path")
    end
    aggregate = _aggregate(hashes)
    occursin("\"aggregate_sha256\": \"$aggregate\"", text) ||
        error("Lock 007 aggregate mismatch")
    return aggregate
end

function dry_run()
    config = validate_execution_design_007()
...
function create_execution_lock_007()
    isfile(LOCK_PATH) && error("Execution Lock 007 already exists; it is immutable")
    config = validate_execution_design_007()
    _assert_prelock_state(config)
    hashes = _hashes()
...
function main(args = ARGS)
    length(args) == 1 || error(
        "usage: lock_financial_strategy_library_panel_v1_execution_007.jl --dry-run|--lock|--check",
    )
    mode = only(args)
    mode == "--dry-run" && return println("Lock 007 dry run passed: ", dry_run())
    mode == "--lock" && return println("Lock 007 created: ", create_execution_lock_007())
    mode == "--check" && return println("Lock 007 valid: ", verify_execution_lock_007())
...
if abspath(PROGRAM_FILE) == @__FILE__
    LockFinancialStrategyLibraryPanelV1Execution007.main()
end
