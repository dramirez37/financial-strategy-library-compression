...
This section preserves the full canonical numerical records from the preprint
program and supports the journal article's condensed economic section. The
tables below are typeset directly from the committed
...
The compact stationary table is included to make the record self-contained.

\newcommand{\recordfield}[1]{\expandafter\path\expandafter{#1}}
\newcolumntype{K}[1]{>{\raggedright\arraybackslash}p{\dimexpr #1\linewidth-2\tabcolsep\relax}}

\subsection{Convergence history and stationary solution}

\begin{figure}[htbp]
  \centering
  \input{figures/unified_canonical_convergence}
...
\scriptsize
\csvreader[
  longtable={@{}K{.07}K{.05}K{.085}K{.07}K{.07}K{.085}K{.07}K{.07}K{.12}K{.07}K{.07}K{.07}@{}},
  table head={\caption{All pairwise resource-price relationships and crossing candidates.}
    \label{tab:supp-pairwise-crossings}\\
    \toprule
    Schedule & Belief & Left library & Left value & Left display burden \(\widetilde W\) & Right library &
    Right value & Right display burden \(\widetilde W\) & Relation & Price & Nonneg. & Globally active\\
    \midrule\endfirsthead
    \toprule
    Schedule & Belief & Left library & Left value & Left display burden \(\widetilde W\) & Right library &
    Right value & Right display burden \(\widetilde W\) & Relation & Price & Nonneg. & Globally active\\
