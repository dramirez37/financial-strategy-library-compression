...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 012 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, phase-specific unused-field fixtures, and independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
...
| `make aor-financial-panel-v1-continue` | Starting from the passing structural audit, run or resume postdecision evaluation, full result audit, eight-thread Parquet analysis, and the independent analysis audit. | Medium to long on the first pass; restart cost is bounded by terminal partitions. | Local licensed CRSP/WRDS delivery plus completed ignored structural artifacts. | Three local-only source partitions, 180 postdecision records, result audit, 180 analysis partitions, compacted analysis tables, and analysis audit. |
