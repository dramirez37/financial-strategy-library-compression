...
BUILD_DIR="$ROOT/../build/online_supplement"
STDOUT_LOG="$BUILD_DIR/latexmk.stdout.log"

for tool in latexmk pdflatex rg; do
...
if [[ ! -f "$LOG_FILE" || ! -f "$BUILD_DIR/main.pdf" ]]; then
    echo "Supplement build did not produce the expected log and PDF." >&2
    exit 1
fi
...
if rg -n \
    ...
    "$LOG_FILE"; then
    echo "Supplement build contains a broken citation, reference, or missing-file warning." >&2
    exit 1
fi

echo "Supplement build passed: $BUILD_DIR/main.pdf"
