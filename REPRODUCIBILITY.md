...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 013 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, phase-specific unused-field and terminal-delisting-pending fixtures, and independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
...
| `make aor-financial-panel-v1-continue` | Starting from the passing structural audit, run or resume postdecision evaluation under Lock 013, full result audit, eight-thread Parquet analysis, and the independent analysis audit. A terminal `DP` return remains missing and makes the affected origin's secondary diagnostic unavailable rather than imputed. | Medium to long on the first pass; restart cost is bounded by terminal partitions. | Local licensed CRSP/WRDS delivery plus completed ignored structural artifacts. | Three local-only source partitions, 180 postdecision records including explicit unavailable records, result audit, 180 analysis partitions, compacted analysis tables, and analysis audit. |
