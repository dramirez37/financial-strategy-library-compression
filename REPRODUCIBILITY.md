...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 011 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, and independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
