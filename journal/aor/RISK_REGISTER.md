...
| M-03 | The SC-GAP family is generalized beyond the rule or order actually proved. | Medium | High | The human proof and exact fixtures establish ratio `k / (1 + epsilon)` for rechecked heaviest-safe-first. Exhaustive small-order validation also shows that singleton-first orders reach the optimum. | State the named rule, identity closure, exact weights, and order boundary every time. Do not convert an existential bad irreducible endpoint into an every-order or every-heuristic claim. |
| M-04 | A negative guarantee for heaviest-safe-first is misreported as a positive or generic greedy approximation theorem. | High | Critical | SC-GAP proves that one named deletion rule has no constant guarantee. It says nothing adverse about forward weighted-cover construction and supplies no positive factor for another method. | Keep negative and positive approximation statements separate. Any positive guarantee needs its own algorithm, assumptions, proof, and oracle tests. |
| M-05 | Preprocessing is applied outside its proved covering scope, or equal-weight strict dominance is reported as preserving every optimizer identity. | Medium | Critical | SC-PRE proves exact residual rules only for the pure positive-weight identity-closure tagged cover. Its adversarial equal-weight dominance fixture preserves optimum value but loses one original optimizer identity; duplicate equal-weight coverage is instead recoverable through an explicit substitution map. | Freeze source tags, propagate mandatory selections first, retain exact audit and reconstruction maps, and require the original frontier/closure/burden/inactive certificate after lifting. Do not reuse these rules for nonidentity closure or identity-sensitive side constraints without new proofs. |
...
| I-07 | Standalone oracle modules and the package schema diverge semantically. | Low to medium | High | `journal-compression-instance-v1` now adapts exact raw/tagged models and the `ResourceOptimization`-compatible mask interface without importing the oracle type. Targeted tests cover the existing adversarial generator, but later algorithms could bypass the adapter. | Require every journal algorithm to consume `JournalCompressionInstance`; keep `ResourceOptimization` independent and compare through its tested adapter rather than unifying implementations. |
| I-08 | Nondeterministic ordering changes ties, selected IDs, hashes, or result rows. | Low for schema inputs; medium for future results | High | The common schema canonicalizes identifiers and requirements, declares tie handling, uses sorted TOML, and rejects unstable ordering. No common result schema is locked yet. | Preserve schema order in every algorithm. The future result schema must serialize complete tie sets or name its deterministic selector separately. |
...
| I-10 | Result-schema fragmentation prevents cross-algorithm comparison despite a common input schema. | Medium | Medium | `journal-compression-instance-v1` now supplies canonical validated inputs and deterministic hashes, but current scripts still have private JSON/CSV result writers. | Lock a separate versioned result schema before outcomes. Reference the common instance hash, store exact values canonically, and keep runtime fields separate. |
...
| Parametric local/global gap family fixtures | Low for the committed grid; exponential beyond it | The human proof is complete. Exact validation covers a bounded rational grid, all active-library masks in that grid, and all deletion orders only through the declared small `k`; the materialized oracle intentionally caps `k` because it stores an exponential value table. |
| Preprocessing equivalence audit | Low for the implemented small oracle; unmeasured for scale | The deterministic targeted gate exhausts original and residual selections for 512 small exact systems and passes 9,433 assertions. Runtime and memory behavior at study scale remain unmeasured and must be registered before benchmarking. |
