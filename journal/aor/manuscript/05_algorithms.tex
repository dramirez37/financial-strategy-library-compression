...
Maximum immediate release is retained as an API alias and as a historical label
in the locked benchmark because those raw records are immutable. It is not an
independent scientific baseline, and coincident results under the two names are
not corroborating replications.
...
The suite does not include a certified multi-exchange neighborhood or an LP
relaxation baseline. Accordingly, the local--global theorem isolates the named
heaviest-safe-first rule and one-deletion irreducibility; it says nothing about
stronger local search. Adding those baselines after seeing the locked benchmark
would be post hoc and is reserved for a separately registered study.
