...
The global problem also has a sharp computational boundary. Under identity
closure, exact frontier--closure preservation is a weighted hitting-set
problem over frontier-attainer and module-carrier obligations. The associated
decision problem is NP-complete, and the minimum-weight problem is NP-hard,
even under unit weights. The reduction is purely finite and exact: it uses no
Bellman approximation, stochastic oracle, mixed-integer tolerance, or
detectability assumption. This result explains why a locally checkable safe
deletion certificate does not by itself yield a generally efficient global
optimizer.
...
\begin{enumerate}
\item \emph{Raw-to-compressed state, exact safe compression, and complexity.}
...
frontier and closure is attained. Under identity closure, the exact decision
problem is NP-complete by a weight-preserving set-cover reduction
(\cref{thm:raw-to-compressed-projection,thm:minimum-safe-compression,%
thm:aor-safe-compression-complexity}).
...
A deletion-irreducible endpoint need not minimize global burden, while the
specific heaviest-safe-first deletion rule has no constant approximation
guarantee on a parameterized identity-closure family.  This negative result
does not apply to every deletion order.  Frontier-only pruning can attain the
complete normalized delayed-bridge loss
(\cref{thm:local-global-safe-compression,thm:aor-heaviest-safe-first-gap,%
...
boundary cases; bounded deterministic heuristics expose the algorithmic
boundary without claiming an approximation guarantee; registered randomized
stress tests exercise the mechanisms in frozen finite generators; and
retrospective financial audits compare stepwise-safe, HiGHS-reported
minimum-resource, and frontier-only libraries without using held-out
descendant quality in a pruning rule or objective
...
its final subsection proves the finite complexity boundary;
