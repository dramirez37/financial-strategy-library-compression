...
function audit_analysis(; write_report::Bool = true, progress_io = stderr)
    ...
    amendment["amendment_id"] == "AMENDMENT_021" || error("active analysis amendment changed")
    paths = _paths(config)
    errors = String[]
    ...
    get(manifest, "resumed_partition_count", -1) == 0 ||
        push!(errors, "Amendment 021 requires all analysis partitions to be rematerialized")
