module LockFinancialStrategyLibraryPanelV2Execution010

using Dates
using SHA: sha256
using TOML

export create_execution_lock_010, dry_run, verify_execution_lock_010
...
const PREDECESSOR_PATH = joinpath(EXPERIMENT_ROOT, "EXECUTION_LOCK_009.json")
const DESIGN_LOCK_PATH = joinpath(EXPERIMENT_ROOT, "DESIGN_LOCK_004.json")
const LOCK_PATH = joinpath(EXPERIMENT_ROOT, "EXECUTION_LOCK_010.json")
const PREDECESSOR_AGGREGATE =
    "28a5f48f48429a01e2de3d3275ff14ff7e76a0369ae190a0accd67866b7f9df5"
...
const REQUIRED_FILES = (
    ...
    "experiments/financial_strategy_library_panel_v2/EXECUTION_LOCK_009.json",
    ...
    "experiments/financial_strategy_library_panel_v2/amendments/EXECUTION_AMENDMENT_009.md",
    "experiments/financial_strategy_library_panel_v2/amendments/EXECUTION_AMENDMENT_009.toml",
    ...
    "julia/scripts/lock_financial_strategy_library_panel_v2_execution_010.jl",
...
function _hashes()
    ...
    for relative in REQUIRED_FILES
        ...
        isfile(path) || error("missing execution-lock-010 input: $relative")
...
function _preflight()
    ...
    occursin("\"aggregate_sha256\": \"$PREDECESSOR_AGGREGATE\"", read(PREDECESSOR_PATH, String)) ||
        error("execution-lock-009 aggregate changed")
    Set(readdir(joinpath(EXPERIMENT_ROOT, "local_data"))) == Set(["master_market_panel"]) ||
        error("execution-lock-010 local-data boundary changed")
    isempty(readdir(joinpath(EXPERIMENT_ROOT, "local_results"))) ||
        error("execution-lock-010 requires clean local results")
    ...
    amendment = TOML.parsefile(joinpath(EXPERIMENT_ROOT, "amendments", "EXECUTION_AMENDMENT_009.toml"))
    amendment["postdecision_outcome_observed_before_amendment"] === false ||
        error("postdecision boundary changed")
    amendment["execution"]["inline_stress_timing"] === false ||
...
function _render(hashes, sources)
    ...
    return """{
  "schema_version": "financial-strategy-library-panel-v2-execution-lock-v10",
  "experiment_id": "financial-strategy-library-panel-v2",
  "execution_amendment_id": "EXECUTION_AMENDMENT_009",
...
function _verify(text, hashes, sources)
    ...
    occursin("\"aggregate_sha256\": \"$aggregate\"", text) || error("execution-lock-010 aggregate mismatch")
    for (path, digest) in hashes
        occursin("\"$path\": \"$digest\"", text) || error("execution-lock-010 file mismatch: $path")
    end
    return aggregate
end

function dry_run()
    _preflight(); hashes = _hashes(); sources = _sources(); return _verify(_render(hashes, sources), hashes, sources)
end

function create_execution_lock_010()
    isfile(LOCK_PATH) && error("execution lock 010 already exists")
