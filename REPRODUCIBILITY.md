...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 017 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, phase-specific unused-field, terminal-delisting-pending, count-only postdecision-profile-adequacy, Julia 1.12 key-shape, and resume-namespace fixtures, with independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
...
| `make aor-financial-panel-v1-continue` | Starting from the passing structural audit, run or resume postdecision evaluation under Lock 017, full result audit, eight-thread Parquet analysis, and the independent analysis audit. Terminal `DP` and insufficient-profile cases remain explicit origin-wide unavailable secondary diagnostics rather than being imputed or deleted; the profile preflight uses exact security-date counts, the 180 analysis keys are flattened before stable sorting, and resume/audit hashes bind their respective key namespaces. | Medium to long on the first pass; restart cost is bounded by terminal partitions and per-instance records. | Local licensed CRSP/WRDS delivery plus completed ignored structural artifacts. | Three local-only source partitions, 180 postdecision records including explicit unavailable records, result audit, 180 analysis partitions, compacted analysis tables, and analysis audit. |
