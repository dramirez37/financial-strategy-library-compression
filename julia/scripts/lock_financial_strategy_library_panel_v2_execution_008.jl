module LockFinancialStrategyLibraryPanelV2Execution008

using Dates
using SHA: sha256
using TOML

export create_execution_lock_008, dry_run, verify_execution_lock_008
...
const PREDECESSOR_PATH = joinpath(EXPERIMENT_ROOT, "EXECUTION_LOCK_007.json")
...
const PREDECESSOR_AGGREGATE =
    "a72da5b42e7cb94049aea41ae1c4d5b3370b06fc8c8067e123dea9285f5f228e"
...
const REQUIRED_FILES = (
    ...
    "experiments/financial_strategy_library_panel_v2/EXECUTION_LOCK_007.json",
    ...
    "experiments/financial_strategy_library_panel_v2/amendments/EXECUTION_AMENDMENT_007.md",
    "experiments/financial_strategy_library_panel_v2/amendments/EXECUTION_AMENDMENT_007.toml",
    ...
    "experiments/financial_strategy_library_panel_v2/amendments/FAILED_EXECUTION_CENSUS_003.toml",
    "julia/Project.toml",
    "julia/Manifest.toml",
    ...
    "julia/scripts/lock_financial_strategy_library_panel_v2_execution_008.jl",
...
function _hashes()
    ...
    for relative in REQUIRED_FILES
        ...
        isfile(path) || error("missing execution-lock-008 input: $relative")
...
function _preflight()
    ...
    amendment = TOML.parsefile(joinpath(EXPERIMENT_ROOT, "amendments", "EXECUTION_AMENDMENT_007.toml"))
    amendment["master_manifest_written_before_amendment"] === false ||
        error("master-manifest boundary changed")
    ...
    amendment["correction"]["value_or_order_change"] === false ||
        error("concurrency correction changed scientific ordering")
    amendment["parallel_replay"]["passed"] === true || error("parallel replay did not pass")
    census = TOML.parsefile(joinpath(EXPERIMENT_ROOT, "amendments", "FAILED_EXECUTION_CENSUS_003.toml"))
    census["local_data"]["file_count"] == 102 || error("attempt-007 data census changed")
...
function _render(hashes, sources)
    ...
    return """{
  "schema_version": "financial-strategy-library-panel-v2-execution-lock-v8",
  "experiment_id": "financial-strategy-library-panel-v2",
  "execution_amendment_id": "EXECUTION_AMENDMENT_007",
  ...
  "file_local_thread_state": true,
  "parallel_replay_passed": true,
...
function _verify(text, hashes, sources)
    ...
    occursin("\"predecessor_execution_lock_aggregate_sha256\": \"$PREDECESSOR_AGGREGATE\"", text) ||
        error("execution-lock-008 predecessor mismatch")
    occursin("\"aggregate_sha256\": \"$aggregate\"", text) || error("execution-lock-008 aggregate mismatch")
    for (path, digest) in hashes
        occursin("\"$path\": \"$digest\"", text) || error("execution-lock-008 file mismatch: $path")
    end
    return aggregate
end
...
function create_execution_lock_008()
    isfile(LOCK_PATH) && error("execution lock 008 already exists")
