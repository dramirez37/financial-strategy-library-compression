...
\begin{figure}[htbp]
  \centering
  \input{figures/unified_canonical_transition}
...
\centering
\small
\input{tables/main_canonical_stationary_solution}
...
order \((I,A,B,D)\), their primary-model weight vectors are equal-active
\((0,1,1,1)\), carrier-heavy \((0,2,2,1)\), and descendant-heavy
\((0,1,1,2)\), consistently with \(w_I=w_{s_0}=0\).  The benchmark artifacts
display a common mandatory resource unit in addition to that formal burden:
\begin{equation}\label{eq:canonical-display-burden}
 \widetilde W(L):=1+W(L).
\end{equation}
Thus the displayed schedule vectors are \((1,1,1,1)\), \((1,2,2,1)\), and
\((1,1,1,2)\).  If \(\widetilde V^\star\) and \(\widetilde J^\star\) denote
the benchmark objectives written in this display convention, then
\begin{align}
 \widetilde V^\star(b;\widetilde B)
 &=V^\star(b;\widetilde B-1), && \widetilde B\ge1,\label{eq:canonical-display-capacity}\\
 \widetilde J^\star(b;\lambda)
 &=J^\star(b;\lambda)-\lambda.\label{eq:canonical-display-penalty}
\end{align}
The translation shifts capacity axes by one unit and penalized-value levels by
\(-\lambda\).  It leaves optimal libraries, switching prices, breakpoint
order, and absolute burden reductions unchanged.  The capacity and penalty
theorems, and their Lean counterparts, apply to \(W\), not to
\(\widetilde W\).
...
two-thirds, or two-fifths of displayed source burden \(\widetilde W\).  These
are exact finite enumeration certificates, not consequences of a greedy
rule.  The complete
...
The hard-capacity optimizer is evaluated at every attainable displayed burden
\(\widetilde B\).  Equal active and carrier heavy jump directly from \(I\) to
\(I+D\).  Descendant heavy passes through the tied carrier pair \(I+A\) or
\(I+B\); the resulting capacity complements make the second value increment
exceed the first.
...
For each schedule and belief, the displayed penalized problem exhausts the
eight affine branches \(V(b,L)-\lambda\widetilde W(L)\).  Only \(I+D\) and
\(I\) are globally active for \(\lambda>0\).  Equal active and carrier heavy
switch at \(1229/288\) from low belief and \(2089/288\) from high belief.
Descendant heavy doubles the relevant burden difference, so its breakpoints
are \(1229/576\) and \(2089/576\).  At a breakpoint both libraries are
optimal; away from it the optimizer is unique after removal of redundant
equal-state libraries.
...
\begin{figure}[p]
  ...
  (a) plots \(\widetilde V^\star(b;\widetilde B)\) at and between attainable
  exact displayed burdens as right-continuous stairs.  Panel (b) plots the
  exact active affine branches of \(\widetilde J^\star(b;\lambda)\) over the
  displayed range.  Panel (c) plots displayed burden \(\widetilde W\) under
  the declared right-continuous minimum-burden tie rule; open endpoints are
  excluded and filled endpoints are selected.  Here
  \(\widetilde W=1+W\) includes the common mandatory display unit; the formal
  model and Lean theorem use \(w_{s_0}=0\).  Equal-active and carrier-heavy
  schedules coincide.  Source values are exact
  \texttt{Rational\{BigInt\}}; only TeX mark placement uses rendered
