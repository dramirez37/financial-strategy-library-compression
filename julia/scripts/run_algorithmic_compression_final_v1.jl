module AlgorithmicCompressionFinalV1
...
include(joinpath(@__DIR__, "lock_algorithmic_compression_postpilot_v1.jl"))
using .LockAlgorithmicCompressionPostpilotV1: verify_postpilot_lock
...
const FINAL_RUN_SCHEMA_VERSION = "algorithmic-compression-final-run-v1"
const FINAL_JOB_SCHEMA_VERSION = "algorithmic-compression-final-job-v1"
const MEMORY_LIMIT_BYTES = 16 * 1024^3
const HEURISTICS = (
    :weighted_greedy,
    :cardinality_greedy,
...
function _execution_lock_hashes()
    paths = (
        "experiments/algorithmic_compression_v1/DESIGN_LOCK.json",
        "experiments/algorithmic_compression_v1/DESIGN_LOCK_AMENDMENT_001.json",
        "experiments/algorithmic_compression_v1/ANALYSIS_PLAN.md",
        "experiments/algorithmic_compression_v1/REPORTING_RULES.md",
        "experiments/algorithmic_compression_v1/registry/INSTANCE_REGISTRY.csv",
        "experiments/algorithmic_compression_v1/registry/SEED_REGISTRY.csv",
        "experiments/configs/algorithmic_compression_v1.toml",
        "julia/Project.toml",
        "julia/Manifest.toml",
        "julia/scripts/run_algorithmic_compression_final_v1.jl",
        "julia/scripts/run_algorithmic_compression_final_worker_v1.jl",
        "julia/scripts/audit_algorithmic_compression_final_v1.jl",
        "julia/test/run_algorithmic_compression_final_runner_tests.jl",
        "julia/test/test_algorithmic_compression_final_runner_v1.jl",
        "Makefile",
    )
...
function _execution_lock_text(
    ...
    return """{
  "schema_version": "algorithmic-compression-final-execution-start-lock-v1",
  "experiment_id": "registered-algorithmic-compression-benchmark-v1",
  "created_at_utc": "$created_at",
  "committed_design_lock_verified": true,
  "committed_postpilot_lock_verified": true,
...
function _hardware_environment()
    ...
    return Dict{String,Any}(
        "schema_version" => "algorithmic-compression-final-environment-v1",
...
function _verify_environment(environment)
    ...
    environment["julia_threads"] == 1 || error("final run requires one Julia thread")
...
function _supervise(job, paths, time_limit)
    ...
    command = `$(Base.julia_cmd()) --startup-file=no --project=$(joinpath(REPOSITORY_ROOT, "julia")) $WORKER_PATH $(paths.job)`
...
function _run_one(job)
    ...
    if isfile(paths.worker)
        ...
        return record
    elseif any(isfile, (paths.job, paths.handshake, paths.stdout, paths.stderr))
...
function _generation_failure_record(spec, seed, unit, position, time_limit, generation_path)
    return Dict{String,Any}(
        "schema_version" => FINAL_RUN_SCHEMA_VERSION,
        "experiment_id" => "registered-algorithmic-compression-benchmark-v1",
        "terminal" => true,
        ...
        "algorithm_execution_position" => position,
...
function _expand_generation_failure!(spec, seed, units, config, generation_path)
...
function _write_run_status(registries)
    ...
    _replace_toml(
        ...
        Dict{String,Any}(
            "schema_version" => "algorithmic-compression-final-execution-state-v1",
...
function validate_final_runner_readiness()
    ...
    Threads.nthreads() == 1 || error("final runner requires one Julia thread")
...
function run_final_benchmark()
    ...
    for row in final_pairs
        ...
        units = _registered_run_units(spec)
        rotation = mod(row.registry_position - 1, length(units))
        ordered = circshift(units, -rotation)
        if !isfile(instance_path)
            if isfile(generation_path)
                ...
                if generation["status"] == "GENERATED"
                    ...
                    _expand_generation_failure!(spec, seed, ordered, config, generation_path)
                    _write_run_status(registries)
        ...
        for (position, unit) in enumerate(ordered)
            ...
            _run_one(job)
            completed += 1
            completed % 50 == 0 && println("final benchmark terminal records visited=$completed/3907")
        end
        _write_run_status(registries)
    end
...
function main(args = ARGS)
    ...
    error("usage: run_algorithmic_compression_final_v1.jl [--check|--run]")
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    AlgorithmicCompressionFinalV1.main()
end
