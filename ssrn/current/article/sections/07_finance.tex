...
point-in-time source construction, an explicit portfolio grammar, prespecified
...
\subsection{Compression and proposal screening policy}
...
only when the registered simultaneous moving-block bootstrap score for incremental
proposal value exceeds 0.0025 in annual certainty-equivalent return. The policy therefore
treats closure as an option, not a forced trade. The score uses the reflected
resampling statistic documented in Online Resource~1, Section~S8. Its nominal
90\% tuning level has no established finite-sample simultaneous coverage for
this design. We interpret it as a prespecified adoption screen, not a verified
confidence guarantee or protection against an adverse held-out realization.
...
Figure~\ref{fig:financial-results} separates burden reduction and option
availability from the origin-level held-out outcomes.
\input{figures/financial_results}

\subsection{Held-out result: options are not exercises}

The screening policy makes \AORPointInTimeEquityPolicyReplacements\
...
separate statistical decision, and the registered selector can abstain.
...
Figure~\ref{fig:synthetic-results} shows the oracle--learner gap with
uncertainty and a separate view of the low-value regimes.
\input{figures/synthetic_results}
...
threshold-based learner rarely exercises a small option at the
