...
| `make aor-financial-panel-v1-smoke` | Execute the successor Lock 022 command topology on eight synthetic lanes with bounded heavy-stage concurrency, durable newline progress, Parquet round trips, phase-specific unused-field, terminal-delisting-pending, count-only postdecision-profile-adequacy, Julia 1.12 key-shape, exact MIP warm-start substitution, mixed Lock-005/Lock-020 checkpoint recovery, precomputed serialized-file hash reuse, exact environment-successor recovery, and analysis-partition rematerialization fixtures, with independent solution checks. | Short to medium. | Public synthetic fixtures only; no licensed row is opened. | Pass/fail diagnostics; all smoke files are temporary. |
...
| `make aor-financial-panel-v1-continue` | Starting from the passing structural audit, run or resume postdecision evaluation under Lock 022, full result audit, eight-thread Parquet analysis, and the independent analysis audit. Amendments 019--022 replace only the 94 exact matching historical MIP projection failures, prevalidate mixed checkpoint provenance, reuse four terminal Lock-020 MIP rows, require the exact sealed environment-successor fingerprint, require exact MIP--DP burden agreement, refresh all postdecision records, and rematerialize all analysis partitions. Terminal `DP` and insufficient-profile cases remain explicit origin-wide unavailable secondary diagnostics rather than being imputed or deleted. | Medium to long on the first corrected pass; restart cost is bounded by terminal checkpoints, per-instance records, and Parquet partitions. | Local licensed CRSP/WRDS delivery plus completed ignored structural artifacts. | Three local-only source partitions, 180 postdecision records including explicit unavailable records, result audit, 180 rematerialized analysis partitions, compacted analysis tables, and analysis audit. |
